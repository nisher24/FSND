...
# Paginate questions
def paginate_quesions(request, selection):
...
def format_categories(categories):
  ...
  categories_dict = {str(category["id"]) : category["type"] for category in categories_format}
...
def create_app(test_config=None):
  ...
  def get_questions():
    ...
    current_questions = paginate_quesions(request, selection)
  ...
  '''
  # Route for getting a random quiz question within the given category, if provided
  ...
  def get_quizzes():
    ...
    if quiz_category['id'] == 0:
      ...
      category = Category.query.filter(Category.id == int(quiz_category['id'])).one_or_none()

      if category is None:
        abort(404)
      
      # question = Question.query.filter(Question.category == quiz_category['id'], ~Question.id.in_(previous_questions)).order_by(func.random()).first()
